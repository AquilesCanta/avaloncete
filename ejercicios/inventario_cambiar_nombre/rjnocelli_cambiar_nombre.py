...
# PARTE 2: AUTENTICACION -------------------------------------
    ...
    while not terminar_loop_autenticar:
        ...
        if usuario_respuesta == 'crear':
                ...
                try:
                    if nombre_de_usuario not in diccionario_de_usuarios:
                        raise ValueError
                except ValueError:
                    ...
                    while not terminar_loop_autenticar:
                        ...
                        try:
                            if diccionario_de_usuarios[nombre_de_usuario].contraseña != respuesta_de_usuario:
                                raise ValueError
                        except ValueError:
                            print('Su contraseña es incorrecta')
                        else:
                            print(f'{nombre_de_usuario}')
                            usuario_autenticado = diccionario_de_usuarios[nombre_de_usuario]
                            terminar_loop_autenticar = True
...
# PARTE 3: INVENTARIO -------------------------------------
    ...
    while not terminar_loop_programa:
        ...
        if comando == 'cambiar nombre':
            diccionario_de_usuarios[usuario_autenticado.nombre].nombre = cambiar_nombre()   
